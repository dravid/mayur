...
export default class extends React.Component {
	...
	render() {
		if (this.props.session.user) {
			...
			let title = this.props.title ? this.props.title : 'Molim. Prijavite se.';

			return (
				<div className="panel signin">
					<div style={{ textAlign: 'center' }} className="panel-heading">
						<img style={{ width: '250px' }} src='/static/images/logo.png' />
					...
					<div className="panel-body">
						...
						<form id="signin" method="post" action="/auth/signin" onSubmit={this.handleSignInSubmit}>
							...
							<div className="form-group">
								<button className="btn btn-success btn-quirk btn-block">Prijavi se</button>
							</div>
						</form>
						...
						<div className="auth-message">
							<p id="wrong_credentials" style={{ display: 'none' }}>Pogresan email ili šifra</p>
							<p id="invalid_email" style={{ display: 'none' }}>Molimo Vas unesite validan e-mail</p>
							<p id="system_error" style={{ display: 'none' }}>Imamo neke tehničke poteškoće. Molimo Vas pokušajte kasnije.</p>
						</div>
						...
						<div className="form-group">
							<a href="/auth/sign-up"
								className="btn btn-default btn-quirk btn-stroke btn-stroke-thin btn-block btn-sign">
								Niste član? Registrujte se ovde!
							</a>
						</div>
					</div>
				</div>
