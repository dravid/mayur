...
import { Input, Select, message, Upload, Icon, Modal, notification, Button, TreeSelect } from 'antd';
...
class Product extends React.Component {
			...
			let pages = await fetch(`${siteUrl}/api/v1/products/products`, {
				...
				if (response.status === 'item_fetched') {
					...
					this.setState({
						...
						selectedCategory: response.data.selectedCategory ? response.data.selectedCategory : "",
	...
	addProduct = async (event) => {
		...
		fetch('/api/v1/products/products', {
			...
			if (response.status === 'database_error') {
				...
				notificationMessage('success', 'Uspeh', 'Proizvod je uspeno dodat.');
	...
	updateProduct = async () => {
		...
		fetch('/api/v1/products/products', {
			...
			if (response.status === 'database_error') {
				...
				notificationMessage('success', 'Uspeh', 'Proizvod je uspesno izmenjen');
	...
	categorySelectHandler = async (selectedCategory) => {
		for (let i = 0; i < this.state.productsCategories.length; i++) {
			if (this.state.productsCategories[i].category[0].title.toLowerCase() == selectedCategory.toLowerCase()) {
				await this.setState({ treeData: this.state.productsCategories[i].category })
			}
		}
	}
							...
							<div className="panel-body">
								<div style={{ height: '100%', width: "100%" }}>
									...
									<Select
										...
										value={this.state.category ? this.state.category : "Select main category"}
									...
									>
										{this.state.productsCategories.map((item, index) => (
											<Option key={index} value={item.category[0].title.toLowerCase()}>{item.category[0].title}</Option>
										))}
									</Select>
